model <- function(t, state, parms) {
  with(as.list(c(state,parms)), {
    ...
    dPRED <- c2*e2*ELK*PRED/(h2+ELK) - deathPRED*PRED*(1+PRED/z2); # Mass action? Bears and Wolves birth rates were saturation of themselves instead of elk. I fixed this cause Im smart.
    dWOLF <- c1*e1*ELK*WOLF/(h1+ELK) - deathWOLF*WOLF*(1+WOLF/z1); # Bears dont only eat ELKs
    return(list(c(dWILG, dELK, dPRED, dWOLF)))  # De conversiefactor van de beren was hetzelfde als hun death rate, heb ik ook veranderd.
  })
}  

p <- c(rWILG=10,rELK=1.0,K1=20,K2=20,c1=1/20,c2=1/100,e2=6,e1=8,h1=4,h2=7.5,deathWOLF=1/6,deathPRED=1/24,p=1/3,z1=0.7,z2=30)
s <- c(WILG=13,ELK=10.55,PRED=0.84,WOLF=0)
run(tmax=100,tstep=0.01,after="if(t == 30) state[\"WOLF\"] = 41/1553",ymax=1,xlab = "Time (years)", ylab="Density per km2")
